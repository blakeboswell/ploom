...
test_that("oomglm", {
  ...
  y <- glm(f, data = mtcars)
  x <- iter_weight(oomglm(f), mtcars, max_iter = 8L)
  ...
  x2 <- iter_weight(oomglm(f2), mtcars, max_iter = 8L)
...
test_that("weighted oomglm", {
  ...
  x <- iter_weight(x, df, max_iter = 8L)
  ...
  x2 <- iter_weight(x2, mtcars, max_iter = 8L)
...
test_that("oomglm without intercept", {
  ...
  y <- glm(f, data = df)
  x <- iter_weight(oomglm(f), data = df, max_iter = 8L)
...
test_that("weighted oomglm without intercept", {
  ...
  y <- glm(f, data = df, weights = w)
  x <- oomglm(f, weights = ~w)
  x <- iter_weight(x, data = df, max_iter = 8L)
