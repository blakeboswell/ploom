...
When fitting in-memory data the process is similar to `oomlm` but we use the function `iter_weight` instead of `update`.  `iter_weight` fits the model via iterative passes over the data until convergence.
...
The `iter_weight` process can also be implemented directly with `ploom` functions:
