...
The functions `oomlm` and `oomglm` are similar to base `lm` and `glm` for fitting in-memory data.

```{r, eval = TRUE}
w <- oomlm(mpg ~ cyl + disp, data = mtcars)
```

Models are initalized with a call to `oomlm` and updated with `update_oomlm`.   The recommended pattern is to initialize models without referencing the data, then call `update_oomlm` on each data chunk in the exact same way.
...
Separating model initialization and processing of the first data chunk enables functional patterns like `reduce` to take the place of loops.  The below example is equivalent to the above `for` loop.
...
For maximum flexibility, `ploom` also supports providing data on initialization similar to [`biglm`](https://github.com/cran/biglm).
