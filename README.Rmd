...
A collection of tools for __out-of-memory__ and __memory efficient__ linear model fitting for inferential analysis. Provides `oomlm()` and `oomglm()` functions for linear and generalised linear modeling.  Fitting is performed via Alan Miller's AS274 updating QR factorization algorithm which enables models with `p` variables to be fit in `p^2` memory resulting in
