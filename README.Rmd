...
The `ploom` function `oomglm` fits generalized linear models via Iterative Weighted Least Squares (IWLS).  
When fitting in-memory data the process is similar to `oomlm` but we use the function `reweight` instead of `update`.  `reweight` itertively fit until convergence.
